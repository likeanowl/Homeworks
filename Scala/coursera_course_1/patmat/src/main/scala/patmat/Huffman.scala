...
object Huffman {
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
      ...
      def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] =
        if (chars.isEmpty)
          ...
          val found = timesChar(chars.head)
          val rest = timesAcc(chars.tail, acc)
          if
            (rest.contains(found)) rest
          else
            found :: rest
        }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      def insert(in: CodeTree, ex: List[CodeTree]) =
        insertBetween(in, ex.partition(weight(_) < weight(in)))

      def insertBetween[T](tree: T, offsets: (List[T], List[T])) = offsets._1 ::: tree :: offsets._2

      trees match {
        case Nil => trees
        case h :: Nil => trees
        case f :: s :: t => insert(makeCodeTree(f, s), t)
      }
    }
   ...
   */
    def until(singM: List[CodeTree] => Boolean, combM: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (singM(trees)) trees else until(singM, combM)(combM(trees))
    }
